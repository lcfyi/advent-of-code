...
void getOxygenLocation(char map[WIDTH][HEIGHT], int *ox, int *oy)
{
...
int fillOxygen(char map[WIDTH][HEIGHT], int minutes, int x, int y)
{
    if (map[x][y] == '#' || map[x][y] == ' ')
    {
        return 0;
    ...
    if (map[x - 1][y] == '.')
        L = fillOxygen(map, minutes + 1, x - 1, y);
    else
        L = 0;
    if (map[x + 1][y] == '.')
        R = fillOxygen(map, minutes + 1, x + 1, y);
    else
        R = 0;
    if (map[x][y - 1] == '.')
        U = fillOxygen(map, minutes + 1, x, y - 1);
    else
        U = 0;
    if (map[x][y + 1] == '.')
        D = fillOxygen(map, minutes + 1, x, y + 1);
    else
        D = 0;

    return max(minutes, max(L, max(R, max(U, D))));
}
...
int main(void)
{
    ...
    getOxygenLocation(map, &ox, &oy);

    int min = fillOxygen(map, 0, ox, oy);
