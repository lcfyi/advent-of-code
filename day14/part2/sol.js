...
// Recursively find the ore count we need
...
// Binary search for our value
let searchSpace = (min, max, target) => {
  let oreCount = getOreCount(
    new Map(),
    "FUEL",
    min + Math.floor((max - min) / 2)
  );
  if (min === max || Math.floor((max - min) / 2) === 0) {
    return min;
  }
  if (oreCount < target) {
    return searchSpace(min + Math.floor((max - min) / 2), max, target);
  } else if (oreCount > target) {
    return searchSpace(min, min + Math.floor((max - min) / 2), target);
  } else {
    return min;
  }
};

console.log("Max fuel:", searchSpace(1, 100000000, 1000000000000));
